...
public class FrontController extends HttpServlet {
     ...
     * (例:パラメータが action=Customer の場合、actions.CustomerActionオブジェクト)
     * @param request リクエスト
    ...
    private ActionBase getAction(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            //リクエストからパラメータ"action"の値を取得 (例:"Customer"、"Voice")
            String actionString = request.getParameter(ForwardConst.ACT.getValue());

            //該当するActionオブジェクトを作成 (例:リクエストからパラメータ action=Customer の場合、actions.CustomerActionオブジェクト)
            type = Class.forName(String.format("actions.%sAction", actionString));

            //ActionBaseのオブジェクトにキャスト(例:actions.CustomerActionオブジェクト→actions.ActionBaseオブジェクト)
